...
int main() {
  ...
  static float fovness = 45.0f;
  static float aspectness = (float)800 / 600;
  static bool polygoneModeWithLine = false;
  static bool showColorWithPos = true;
  static bool proj_orhto = false;
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    ImGui::NewFrame();
    {
      ...
      ImGui::Checkbox("proj with ortho", &proj_orhto);
      ImGui::SliderFloat("mixness", &mixness, 0.0f, 1.0f);
      ImGui::SliderFloat("fovness", &fovness, 0.0f, 180.0f);
      ImGui::SliderFloat("aspectness", &aspectness, 0.0f, 2.0f);
    ...
    if (!proj_orhto)
      projection =
          glm::perspective(glm::radians(fovness), aspectness, 0.0f, 100.0f);
    else
      projection =
          glm::ortho(-aspectness, aspectness, -1.0f, 1.0f, -5.0f, 5.0f);
